...
extern char *PyOS_Readline(PYFILE *, PYFILE *, char *);
...
{
    return pyfgets(s, size, tok->fp);
}

static int
decoding_feof(struct tok_state *tok)
{
    return pyfeof(tok->fp);
}
...
struct tok_state *
PyTokenizer_FromFile(PYFILE *fp, char *ps1, char *ps2)
{
    struct tok_state *tok = tok_new();
    if (tok == NULL)
        return NULL;
...
tok_stdin_decode(struct tok_state *tok, char **inp)
{
    ...
    if (PySys_GetFile((char *)"stdin", NULL) != pystdin)
        return 0;
...
tok_nextc(register struct tok_state *tok)
{
    for (;;) {
        ...
        if (tok->prompt != NULL) {
            char *newtok = PyOS_Readline(pystdin, pystdout, tok->prompt);
