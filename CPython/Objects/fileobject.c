...
#if defined(HAVE_GETC_UNLOCKED) && !defined(QT_CORE_LIB) && !defined(GD_PYTHON)
#define GETC(f) getc_unlocked(f)
#define FLOCKFILE(f) flockfile(f)
#define FUNLOCKFILE(f) funlockfile(f)
#else
#define GETC(f) pygetc(f)
#define FLOCKFILE(f)
...
dircheck(PyFileObject* f)
{
    ...
    if (f->f_fp == NULL)
        return f;
    if (fstat(pyfileno(f->f_fp), &buf) == 0 &&
        S_ISDIR(buf.st_mode)) {
...
static PyObject *
fill_file_fields(PyFileObject *f, PYFILE *fp, PyObject *name, char *mode,
                 int (*close)(PYFILE *))
{
    assert(name != NULL);
    assert(f != NULL);
...
{
    int sts = 0;
    int (*local_close)(PYFILE *);
    PYFILE *local_fp = f->f_fp;
...
PyObject *
PyFile_FromFile(PYFILE *fp, char *name, char *mode, int (*close)(PYFILE *))
{
    PyFileObject *f;
    PyObject *o_name;
...
PyFile_FromString(char *name, char *mode)
{
    ...
    f = (PyFileObject *)PyFile_FromFile((PYFILE *)NULL, name, mode, NULL);
    if (f != NULL) {
...
#if !defined(QT_CORE_LIB) && !defined(GD_PYTHON)
        else {
...
static int
_portable_fseek(PYFILE *fp, Py_off_t offset, int whence)
{
#if defined(QT_CORE_LIB) || defined(GD_PYTHON)
    return pyfseek(fp, offset, whence);
#elif !defined(HAVE_LARGEFILE_SUPPORT)
...
file_truncate(PyFileObject *f, PyObject *args)
{
    ...
    FILE_BEGIN_ALLOW_THREADS(f)
    errno = 0;
    ret = ftruncate(pyfileno(f->f_fp), newsize);
    FILE_END_ALLOW_THREADS(f)
    if (ret != 0)
        goto onioerror;
#endif /* !MS_WINDOWS */
...
file_tell(PyFileObject *f)
{
    ...
    if (f->f_skipnextlf) {
        ...
        if (c == '\n') {
            ...
            pos++;
            f->f_skipnextlf = 0;
        } else if (c != EOF) pyungetc(c, f->f_fp);
    }
#if !defined(HAVE_LARGEFILE_SUPPORT)
...
file_fileno(PyFileObject *f)
{
    if (f->f_fp == NULL)
        return err_closed();
    return PyInt_FromLong((long) pyfileno(f->f_fp));
}
...
file_isatty(PyFileObject *f)
{
    ...
    if (f->f_fp == NULL)
        return err_closed();
    FILE_BEGIN_ALLOW_THREADS(f)
    res = isatty((int)pyfileno(f->f_fp));
    FILE_END_ALLOW_THREADS(f)
...
new_buffersize(PyFileObject *f, size_t currentsize)
{
    ...
    if (fstat(pyfileno(f->f_fp), &st) == 0) {
        ...
        pos = lseek(pyfileno(f->f_fp), 0L, SEEK_CUR);
        if (pos >= 0) {
            pos = pyftell(f->f_fp);
        }
        if (pos < 0)
...
#define CHUNKSIZE 1000
    ...
    for (index = 0; ; index += CHUNKSIZE) {
        ...
        for (i = 0; i < j; i++) {
            ...
            nwritten = pyfwrite(PyString_AS_STRING(line),
                              1, len, f->f_fp);
            if (nwritten != len) {
...
*/
char *
Py_UniversalNewlineFgets(char *buf, int n, PYFILE *stream, PyObject *fobj)
{
    char *p = buf;
    int c;
    int newlinetypes = 0;
