...
static int
internal_print(PyObject *op, PYFILE *fp, int flags, int nesting)
{
    int ret = 0;
    if (nesting > 10) {
...
#endif
    pyclearerr(fp); /* Clear any previous error condition */
    if (op == NULL) {
...
int
PyObject_Print(PyObject *op, PYFILE *fp, int flags)
{
...
#endif
        pyfprintf(pystderr, "object  : ");
#ifdef WITH_THREAD
        gil = PyGILState_Ensure();
#endif
        (void)PyObject_Print(op, pystderr, 0);
#ifdef WITH_THREAD
 ...
 */
void
_Py_PrintReferenceAddresses(PYFILE *fp)
{
    PyObject *op;
