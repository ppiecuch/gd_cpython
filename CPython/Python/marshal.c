...
#define w_byte(c, p) if (((p)->fp)) pyputc((c), (p)->fp); \
...
void
PyMarshal_WriteLongToFile(long x, PYFILE *fp, int version)
{
    WFILE wf;
    wf.fp = fp;
    wf.error = WFERR_OK;
...
void
PyMarshal_WriteObjectToFile(PyObject *x, PYFILE *fp, int version)
{
    WFILE wf;
    wf.fp = fp;
    wf.error = WFERR_OK;
...
#define r_byte(p) ((p)->fp ? pygetc((p)->fp) : rs_byte(p))
...
int
PyMarshal_ReadShortFromFile(PYFILE *fp)
{
    RFILE rf;
    assert(fp);
    rf.fp = fp;
...
long
PyMarshal_ReadLongFromFile(PYFILE *fp)
{
    RFILE rf;
    rf.fp = fp;
    rf.strings = NULL;
...
static off_t
getfilesize(PYFILE *fp)
{
#ifdef QT_CORE_LIB
    return pyffilesize(fp);
#else
    struct stat st;
    if (fstat(pyfileno(fp), &st) != 0)
        return -1;
    else
        return st.st_size;
#endif
 ...
 */
PyObject *
PyMarshal_ReadLastObjectFromFile(PYFILE *fp)
{
...
#define REASONABLE_FILE_LIMIT (1L << 18)
    ...
    if (filesize > 0 && filesize <= REASONABLE_FILE_LIMIT) {
        ...
        if (pBuf != NULL) {
            ...
            n = pyfread(pBuf, 1, (int)filesize, fp);
...
PyObject *
PyMarshal_ReadObjectFromFile(PYFILE *fp)
{
    RFILE rf;
    PyObject *result;
    rf.fp = fp;
