...
PyObject *
PyImport_ExecCodeModuleEx(const char *name, PyObject *co, char *pathname)
{
    PyObject *modules = PyImport_GetModuleDict();
    PyObject *m, *d, *v;
...
static PyObject *
    load_compiled_module(char *name, char *cpathname, PYFILE *fp)
{
    long magic;
    PyCodeObject *co;
    PyObject *m;
...
static PyCodeObject *
parse_source_module(const char *pathname, PYFILE *fp)
{
...
static PyObject *
load_source_module(char *name, char *pathname, PYFILE *fp)
{
    struct stat st;
    PYFILE *fpc;
...
find_module(char *fullname, char *subname, PyObject *path, char *buf,
            size_t buflen, PYFILE **p_fp, PyObject **p_loader)
{
    Py_ssize_t i, npath;
    size_t len, namelen;
    ...
    if (fp == NULL) {
        PyErr_Format(PyExc_ImportError,
                     "No module find named %.200s", name);
        return NULL;
    }
    *p_fp = fp;
    return fdp;
...
static PyObject *
load_module(char *name, PYFILE *fp, char *pathname, int type, PyObject *loader)
{
    PyObject *modules;
    PyObject *m;
    int err;
          ...
          Py_ssize_t *p_buflen)
{
    ...
    *p_buflen = p+len-buf;
    ...
    if (result == Py_None) {
        Py_DECREF(result);
        PyErr_Format(PyExc_ImportError,
                     "No module load named %.200s", name);
        return NULL;
    }
...
call_find_module(char *name, PyObject *path)
{
    ...
    if (fdp == NULL)
        return NULL;
    if (fp != NULL) {
        fob = PyFile_FromFile(fp, pathname, fdp->mode, pyfclose);
        if (fob == NULL)
            return NULL;
    }
    else {
