...
static int
_check_and_flush (PYFILE *stream)
{
  int prev_fail = pyferror (stream);
  return pyfflush (stream) || prev_fail ? EOF : 0;
}
...
#define SET_SYS_FROM_STRING(key, value)                 \
     ...
     * from that procedure.
     */
    sysin = PyFile_FromFile(pystdin, "<stdin>", "r", NULL);
    sysout = PyFile_FromFile(pystdout, "<stdout>", "w", _check_and_flush);
    syserr = PyFile_FromFile(pystderr, "<stderr>", "w", _check_and_flush);
    if (PyErr_Occurred())
        return NULL;
...
static void
mywrite(char *name, PYFILE *fp, const char *format, va_list va)
{
    PyObject *file;
...
PySys_WriteStdout(const char *format, ...)
{
    ...
    va_start(va, format);
    mywrite("stdout", pystdout, format, va);
    va_end(va);
}
...
PySys_WriteStderr(const char *format, ...)
{
    ...
    va_start(va, format);
    mywrite("stderr", pystderr, format, va);
    va_end(va);
}
