...
static int
deque_tp_print(PyObject *deque, PYFILE *fp, int flags)
{
    PyObject *it, *item;
    ...
    if (((dequeobject *)deque)->maxlen == -1)
        ...
        pyfprintf(fp, "], maxlen=%" PY_FORMAT_SIZE_T "d)", ((dequeobject *)deque)->maxlen);
    Py_END_ALLOW_THREADS
    return 0;
}
...
static int
defdict_print(defdictobject *dd, PYFILE *fp, int flags)
{
    int sts;
    Py_BEGIN_ALLOW_THREADS
    pyfprintf(fp, "defaultdict(");
    Py_END_ALLOW_THREADS
    if (dd->default_factory == NULL) {
        Py_BEGIN_ALLOW_THREADS
        pyfprintf(fp, "None");
