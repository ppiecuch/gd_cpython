...
sock_makefile(PySocketSockObject *s, PyObject *args)
{
    ...
    if (((fd = _open_osfhandle(s->sock_fd, _O_BINARY)) < 0) ||
        ((fd = dup(fd)) < 0) || ((fp = pyfdopen(fd, mode)) == NULL))
#else
    if ((fd = dup(s->sock_fd)) < 0 || (fp = pyfdopen(fd, mode)) == NULL)
#endif
    {
        if (fd >= 0)
            SOCKETCLOSE(fd);
    ...
    f = PyFile_FromFile(fp, "<socket>", mode, pyfclose);
    if (f != NULL)
...
socket_gethostbyaddr(PyObject *self, PyObject *args)
{
    ...
    PyObject *ret;
#ifdef __psp2__
    PyErr_SetString(socket_error, "Not Supported on PSVita");
    return NULL;
#endif
#ifdef HAVE_GETHOSTBYNAME_R
    struct hostent hp_allocated;
