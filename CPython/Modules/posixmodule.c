...
posix_fdopen(PyObject *self, PyObject *args)
{
    ...
    f = PyFile_FromFile(fp, "<fdopen>", orgmode, pyfclose);
    if (f != NULL)
        PyFile_SetBufSize(f, bufsize);
    return f;
}
...
posix_tmpfile(PyObject *self, PyObject *noargs)
{
    ...
    if (fp == NULL)
        return posix_error();
    return PyFile_FromFile(fp, "<tmpfile>", "w+b", pyfclose);
}
#endif
...
#else
# ifdef __APPLE__
    if ((name = mktemp("_pytmp.XXXXXX")))
        strncpy(buffer, name, L_tmpnam);
# else
    name = tmpnam(buffer);
# endif
#endif
